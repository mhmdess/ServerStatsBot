...
class YourBot(telepot.Bot):
    ...
    def on_chat_message(self, msg):
        ...
        # this will tell you your chat_id
        ...
        if chat_id in adminchatid:  # Store adminchatid variable in tokens.py
            if content_type == 'text':
                if msg['text'] == '/stats' and chat_id not in shellexecution:
                    print("chat_id:" + str(chat_id) + " selected /stats")
                    ...
                    memtotal = "Total memory: %.2f GB " % (
                        memory.total / 1000000000)
                    ...
                    clearall(chat_id)
                    bot.sendMessage(
                        chat_id, "All operations stopped.", reply_markup=hide_keyboard)
                elif msg['text'] == '/setpoll' and chat_id not in setpolling:
                    print("chat_id:" + str(chat_id) + " selected /setpoll")
                    bot.sendChatAction(chat_id, 'typing')
                    setpolling.append(chat_id)
                    bot.sendMessage(
                        chat_id, "Send me a new polling interval in seconds? (higher than 10)", reply_markup=stopmarkup)
                    ...
                    try:
                        ...
                        bot.sendMessage(
                            chat_id, "Please send a proper numeric value higher than 10.")
                elif msg['text'] == "/shell" and chat_id not in shellexecution:
                    print("chat_id:" + str(chat_id) + " selected /shell")
                    bot.sendMessage(
                        chat_id, "Send me a shell command to execute", reply_markup=stopmarkup)
                    ...
                    if output != b'':
                        ...
                        bot.sendMessage(chat_id, "No output.",
                                        disable_web_page_preview=True)
                elif msg['text'] == "/ping" and chat_id not in shellexecution:
                    print("chat_id:" + str(chat_id) + " selected /ping")
                    bot.sendChatAction(chat_id, 'typing')
                    if(msg['text'] != b''):
                        p = Popen(' ping -c 4 google.com', shell=True, stdin=PIPE,
                        ...
                        bot.sendMessage(chat_id, "No output.",
                                        disable_web_page_preview=True)
                elif msg['text'].startswith("/ping") and chat_id not in shellexecution:
                    print("chat_id:" + str(chat_id) + " selected /ping")
                    bot.sendChatAction(chat_id, 'typing')
                    if(msg['text'] != b''):
                        p = Popen(' ping -c 4 ' + msg['text'].replace(
                            "/ping", ""), shell=True, stdin=PIPE, stdout=PIPE, stderr=STDOUT, close_fds=True)
                        output = p.stdout.read()
                        if (output != b''):
                            bot.sendMessage(chat_id, output,
                                            disable_web_page_preview=True)
                    ...
                    print("chat_id:" + str(chat_id) + " selected /setmem")
                    bot.sendChatAction(chat_id, 'typing')
                    settingmemth.append(chat_id)
                    bot.sendMessage(
                        chat_id, "Send me a new memory threshold to monitor?", reply_markup=stopmarkup)
                    ...
                    try:
                        ...
                        bot.sendMessage(
                            chat_id, "Please send a proper numeric value below 100.")
                elif chat_id in shellexecution:
                    bot.sendChatAction(chat_id, 'typing')
                    p = Popen(msg['text'], shell=True, stdin=PIPE,
                              stdout=PIPE, stderr=STDOUT, close_fds=True)
                    output = p.stdout.read()
                    if output != b'':
                        bot.sendMessage(chat_id, output,
                    ...
                    print("chat_id:" + str(chat_id) + " selected /memgraph")
                    bot.sendChatAction(chat_id, 'typing')
                    tmperiod = "Last %.2f hours" % (
                        (datetime.now() - graphstart).total_seconds() / 3600)
                    bot.sendPhoto(chat_id, plotmemgraph(
                        memlist, xaxis, tmperiod))
...
while 1:
    if tr == poll:
        ...
        if mempercent > memorythreshold:
            ...
            tmperiod = "Last %.2f hours" % (
                (graphend - graphstart).total_seconds() / 3600)
